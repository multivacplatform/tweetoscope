...
$thedb = $graphdb;
$gexf=$_GET["gexf"];
$max_item_displayed=$_GET["maxpubs"]-1;
$max_authors=5;
$type = $_GET["type"];
$TITLE="ISITITLE";
	...
	if ($total_count<$count_max) {
		...
		if ($to_display){
			...
			if ($count<=$max_item_displayed){
				...
				foreach ($base->query($sql) as $row) {
					$external_link="<a href=http://google.com/webhp?#q=".urlencode('"'.$row['data'].'"')." target=blank>".'</a>';
				...
				$et_al="";
				foreach ($base->query($sql) as $row) {
					$porciones = explode(", ", $row['data']);
					$temp_name = ucwords($porciones[1])." ".ucfirst($porciones[0]);
					$output.=($temp_name).' , ';
					$auth_counter++;
					if($auth_counter==$max_authors) {
						$et_al=" et al.";
						break;
					}
				}
				$output = rtrim($output, " , ").$et_al;
				// get the year
				$output.="</li><br>";
...
$restrains='';
foreach($elems as $elem){
      	$restrains.=' '.$column.'="'.$elem.'" OR ';
    }
$restrains=substr($restrains, 0, -3);
$sql="SELECT COUNT(*) FROM (SELECT id FROM ".$_GET["index"]." where ".$restrains."  COLLATE NOCASE group by id)";
$sql_backup=$sql;
$the_result = "";
foreach ($base->query($sql) as $row) {
	$K= $row['COUNT(*)'];
...
$N=0;
// Count all pubs:
// SELECT COUNT(*) FROM ISITITLE
$sql="SELECT COUNT(*) FROM ISITITLE";
$the_result = "";
foreach ($base->query($sql) as $row) {
	$N= $row['COUNT(*)'];
	break;
...
// SELECT COUNT(*) FROM ISITITLE
// $output .= '<li>'."K: ".$K."</li>";
// $output .= '<li>'."K_query: ".$sql_backup."</li>";
// $output .= '<li>'."N: ".$N."</li>";
$output .= '<span style="display: none;" id="perctg_selection">'.round(($K/$N*100),1)."</span>";
...
echo $news.'<h4> Exemples de publications (parmi '.$K.' pertinentes):</h4><br>'.$output;
//pt - 301 ; 15.30
?>
