...
function sigmaLimits( sigmacanvas ) {
    // pr(sigmacanvas)
    var pw=$( sigmacanvas ).parent().width();
    var ph=$( sigmacanvas ).parent().height();

    $( sigmacanvas ).width(pw);
    $( sigmacanvas ).height(ph);

    // .css("max-width" , $("#rightish").width()+"px")
    var rightpanel_W = $("#rightish").width()
    var rightpanel_H = $("#rightish").height()
    $( ".the_container" ).width( (rightpanel_W*0.9)+"px" )
    $( ".the_container" ).css( "margin-left","5%" )
    if(g!=false)
        g.resize((rightpanel_W*0.8),(rightpanel_H*0.2))
    if(g_mini!=false)
        g_mini.resize($(".irs").width(),60)

    // $('#tips').css( {'font-size': $(document).width()*0.8/100+"px;" });


    return "canvas! w:"+pw+" , h:"+ph;
...
TinaWebJS = function ( sigmacanvas ) {
    ...
    this.initListeners = function (categories, partialGraph) {
        ...
        $(".reboot").click(function(){
            ...
            partialGraph.zoomTo(partialGraph._core.width / 2, partialGraph._core.height / 2, 0.9);
        ...
        $('#sigma-example').dblclick(function(event) {// using SelectionEngine
            ...
            if(targeted.length>0) {
                ...
                cancelSelection(false);
                MainTwitterApp( ["global warming"] , null  )
            }
