...
if(RES["OK"]) {
    ...
    if (the_data.clusters)
        Clusters = the_data.clusters
    ...
    var en_fr = getCsv( "newinterface/FR-EN.csv" )
    if(en_fr) {
        for(var i in dicts.nodes) {
            dicts.nodes[i]["label_fr"] = (FR_EN__Dict[dicts.nodes[i].label]) ? FR_EN__Dict[dicts.nodes[i].label] : dicts.nodes[i].label;
            dicts.nodes[i]["label_en"] = dicts.nodes[i].label;
            dicts.nodes[i].label = (LA=="FR")?dicts.nodes[i]["label_fr"]:dicts.nodes[i]["label_en"]
        }
    }
    ...
    partialGraph.zoomTo(partialGraph._core.width / 2, partialGraph._core.height / 2, 0.9).draw();
