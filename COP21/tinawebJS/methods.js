...
function htmlfied_tagcloud(elems , limit) {
    ...
    for(var i in elems){
        ...
        if(!isUndef(Nodes[id])){
            ...
            htmlfied_alternode = '<span class="tagcloud-item" style="font-size:'+fontSize+'px;" '+jspart+'>'+ Nodes[id].label+ '</span>';
            oppositesNodes.push(htmlfied_alternode)
        }
    }
    return oppositesNodes
...
function manualForceLabel(nodeid,active) {
	...
	partialGraph._core.graph.nodesIndex[nodeid].forceLabel=active;
...
function updateLeftPanel_fix( sels , oppos ) {
    ...
    namesDIV+='<div id="selectionsBox"><h3>';
    ...
    if(oppos.length>0) {
        ...
        if(getNodeIDs(sels).length>0) {
            ...
            sameNodesDIV+='<div id="opossitesBox">';//tagcloud
            sameNodesDIV+= htmlfied_tagcloud( voisinage , tagcloud_limit).join("\n")
            sameNodesDIV+= '</div>';
    ...
    var logo_app =
        '<br>'+
        '<center>'+
        '  <img width="60%" src="newinterface/wos_logo.png"></img>'+
        '</center>'+
        '<br>';
    $("#logo_app").html(logo_app);
...
function saveGraphIMG(){
        ...
        // var link = document.getElementById('snapicon');
        // link.href = edgesDiv.toDataURL();
        // link.download = "GraphCapture.png";
