...
function dictfyGexf( gexf , categories ){
    ...
    for(i=0; i<nodesNodes.length; i++) {
        ...
        for(j=0; j<nodeNodes.length; j++) {
            ...
            // [ get Label ]
            var rawlabel = nodeNode.getAttribute('label') || id;

            // => we'll keep both label (lowercase) and rawlabel (original)
            //    (should lowerCase but remote data was indexed with original labels)
